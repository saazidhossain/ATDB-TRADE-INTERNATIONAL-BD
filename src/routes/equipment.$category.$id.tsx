...
const specSheetModule = () => import("@/lib/spec-sheet");
    ...
    const breadcrumbLd = {
      ...
      itemListElement: [
        ...
        },
        {
          "@type": "ListItem",
          position: 3,
          name: CATEGORIES[eq.category].label,
          item: `https://www.atdbtrade.com/equipment/${eq.category}`,
        },
        {
          "@type": "ListItem",
          position: 4,
          name: eq.name,
          item: `https://www.atdbtrade.com/equipment/${eq.category}/${eq.id}`,
        },
      ],
    };
    return {
      ...
      links: [
        {
          rel: "canonical",
          href: `https://www.atdbtrade.com/equipment/${params.category}/${params.id}`,
        },
      ],
      scripts: [
...
function EquipmentDetailPage() {
  ...
  const realPhotoState = useAllRealPhotosState(id);
  const { add, items } = useCart();
  const [pdfBusy, setPdfBusy] = useState(false);
  const [pdfError, setPdfError] = useState<string | null>(null);

  // Defensive: if beforeLoad was somehow bypassed and eq is null, bail out.
  if (!eq) {
    return (
      <Layout>
        <div className="container-page py-32 text-center">
  ...
  const realSlots: GallerySlot[] = realPhotos.map((src) => ({
    src,
    captionKey: "gallery.cap.real",
  }));

  const baseSlots: GallerySlot[] =
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      const { generateSpecSheet: gen } = await specSheetModule();
      await gen(eq, runtime);
    } catch (err) {
      ...
      <section className="bg-background py-10 md:py-14">
        <div className="container-page grid gap-10 lg:grid-cols-[1.2fr_1fr]">
          ...
          <div>
            ...
            {pdfError && (
              <p className={`mt-2 text-xs text-destructive ${fontClass}`}>
...
function RealPhotoStatus({
  ...
  if (state.error) {
    return (
      <div
        className={`mt-3 flex items-start justify-between gap-3 rounded-md border border-destructive/30 bg-destructive/10 p-3 text-xs text-destructive ${fontClass}`}
      >
        ...
        <button
          ...
          className="inline-flex shrink-0 items-center gap-1 font-semibold uppercase tracking-wider"
        >
  ...
  return (
    <div
      className={`mt-3 flex items-center justify-between gap-3 rounded-md border border-border bg-muted/40 px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground ${fontClass}`}
    >
      <span>
        {realCount} {t("realPhoto.status.synced")}
      </span>
      <button
        type="button"
        onClick={state.refresh}
        className="inline-flex items-center gap-1 text-safety"
      >
        <RefreshCw className={`h-3.5 w-3.5 ${state.refreshing ? "animate-spin" : ""}`} />{" "}
        {t("realPhoto.status.refresh")}
      </button>
    </div>
  );
}
